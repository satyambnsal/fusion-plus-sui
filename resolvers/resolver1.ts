...
ws.on('message', async (data) => {
  ...
  console.log("Resolver2", message)
  if (message.event === 'newOrder') {
    const { order, signature, srcChainId, extension, secretHash, coinObjectId, hashLock, orderHash } = message.data;
    console.log(`Received new order: ${orderHash}`);
